...
func (r *broker) Publish(ctx context.Context, topic pubsub.Topic, msg interface{}) error {
	...
	return r.channel.PublishWithContext(ctx, r.exchange, topic.String(), false, false, m)
}

func (r *broker) Subscribe(ctx context.Context, topic pubsub.Topic, handler pubsub.Handler, option ...pubsub.SubscribeOption) (pubsub.Subscription, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
